    ...
    <?= $form->field($device, 'manufacturer_id')->dropDownList(
        ...
        [
            ...
            'onchange' => new JsExpression("
                $('.field-opticalattenuator-model_id').removeAttr('style');
                $('.field-opticalattenuator-serial').removeAttr('style');
                $('.field-opticalattenuator-desc').removeAttr('style');
                $.get('" . Url::to(['model/list']) . "&typeId=" . OpticalAttenuator::TYPE . "&manufacturerId=' + $(this).val(), function(data) {
                    $('select#opticalattenuator-model_id').html(data);
                });
            ")
        ]
    )?>

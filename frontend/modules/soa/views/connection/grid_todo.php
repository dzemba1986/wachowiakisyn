        	...
        	'value'=> function($model, $key){
        		if (!is_null($model->task_id)){
        			if (is_object($model->task))
        				return Html::a(date('Y-m-d', strtotime($model->task->start)), Url::to(['/crm/install-task/view-calendar', 'connectionId' => $key]), [
        				    'onclick' => "
        		...
        		elseif ($model->socket > 0)
        			...
        			return Html::a('dodaj', Url::to(['/crm/install-task/view-calendar', 'connectionId' => $key]), [

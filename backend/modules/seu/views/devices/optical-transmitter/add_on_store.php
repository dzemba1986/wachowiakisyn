...
use backend\models\OpticalTransmitter;
    ...
    <?= $form->field($device, 'manufacturer_id')->dropDownList(
        ...
        [
            ...
            'onchange' => new JsExpression("
                $('.field-opticaltransmitter-model_id').removeAttr('style');
                $('.field-opticaltransmitter-serial').removeAttr('style');
                $('.field-opticaltransmitter-mac').removeAttr('style');
                $('.field-opticaltransmitter-desc').removeAttr('style');
                $.get('" . Url::to(['model/list']) . "&typeId=" . OpticalTransmitter::TYPE . "&manufacturerId=' + $(this).val(), function(data) {
                    $('select#opticaltransmitter-model_id').html(data);
                });
            ")
        ]
    )?>
...
$js = <<<JS
$(function() {
    $('.modal-header h4').html('Dodaj nadajnik optyczny');
	 	...
	 	$.post(
	 		...
	 		if(result == 1){
				...
				$('#modal-sm').modal('hide');
	 			...
	 			$.growl.error({ message: 'Błąd dodawania nadajnika optycznego'});
	 		}
	 	}).fail(function(){

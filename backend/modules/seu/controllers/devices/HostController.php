...
use common\models\seu\forms\AddHostForm;
...
class HostController extends DeviceController
{
    ...
    public function actionGetChangeMacScript($id, $newMac) {
        ...
        $host = $this->findModel($id);
        $vlansToIps = $host->vlansToIps;
        $portName = $host->parentPortName;
        
        $conf = [];
        $conf['portName'] = $portName;
        $conf['vlan'] = $vlansToIps[0]['vlan'];
        $conf['mac'] = $host->mac;
        $host->confAttributes = $conf;

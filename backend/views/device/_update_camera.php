...
//      			$.pjax.reload({container: '#subnet-grid-pjax'});
    ...
    if($("#camera-original_name").is(':checked'))
    	$("#camera-name").attr('disabled', true); 
    else { //jeżeli nazwa orginalna nie jest zaznaczona
		var name = $("#camera-name").val()
        
    	$("#camera-name").val(name.replace(/^([\w|\W]{1,})([\[]{1})([\w|\W]{0,})([\]]{1})$/gi, "$3"));
    } 
    
    $("#camera-original_name").change(function() {
        $("#camera-name").attr('disabled', this.checked);
    });
});
</script>

...
rsync -a -e 'ssh -q -p 22222' --size-only --checksum --delete-after $LOCAL_DIR/subnets/ dhcpsync@172.20.4.6:$REMOTE_DIR/ &>> $LOCAL_DIR/log/updateDHCP.log
echo "$CZAS Wykonano synchronizacje plikow dhcp." >> $LOCAL_DIR/log/$CZAS_FILE.updateDHCP.log
sleep 1
if [ `rsync -t -e 'ssh -q -p 22222' --size-only --dry-run dhcpsync@172.20.4.6:$REMOTE_ROOT/ | grep error.lock | wc -l` -gt 0 ]
then
        echo "$CZAS Wystapil blad podczas synchronizacji plikow dhcp!" >> $LOCAL_DIR/log/$CZAS_FILE.updateDHCP_error.log
        rsync -e 'ssh -q -p 22222' --size-only dhcpsync@172.20.4.6:$REMOTE_ROOT/error.lock $LOCAL_DIR/ &>> $LOCAL_DIR/log/$CZAS_FILE.updateDHCP_error.log
fi
rm $LOCK_FILE
sleep 1

...
class m190513_065301_add_connection_package_table extends Migration {
	...
	public function safeUp() {
		...
		$this->createTable('connection_package', [
			'id' => 'serial', 'name' => $this->string(50), 'parent_id' => $this->integer()
		]);
		$this->addPrimaryKey('connection_package_pkey', 'connection_package', 'id');

		$this->insert('connection_package', [
			'id' => 1, 'name' => 'Internet', 'parent_id' => 0
		]);
		$this->insert('connection_package', [
			'id' => 2, 'name' => 'Telefon', 'parent_id' => 0
		]);
		$this->insert('connection_package', [
			'id' => 3, 'name' => 'Telewizja', 'parent_id' => 0
		]);
		$this->insert('connection_package', [
			'id' => 4, 'name' => 'IS', 'parent_id' => 1
		]);
		$this->insert('connection_package', [
			'id' => 5, 'name' => '5G', 'parent_id' => 1
		]);
		$this->insert('connection_package', [
			'id' => 6, 'name' => 'T', 'parent_id' => 2
		]);
		$this->insert('connection_package', [
			'id' => 7, 'name' => 'TP', 'parent_id' => 2
		]);
		$this->insert('connection_package', [
			'id' => 8, 'name' => 'DVB-C', 'parent_id' => 3
		]);
		$this->insert('connection_package', [
			'id' => 9, 'name' => 'IPTV', 'parent_id' => 3
		]);
		$this->insert('connection_package', [
			'id' => 10, 'name' => 'RFoG', 'parent_id' => 3
		]);
		$this->insert('connection_package', [
			'id' => 11, 'name' => 'Mobilny', 'parent_id' => 1
		]);
